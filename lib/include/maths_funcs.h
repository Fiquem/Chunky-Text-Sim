...
inline vec3 rotate(vec3 v, versor q)
{
    // Calculate the conjugate of quaternion
    versor q1 = versor (-q.q[0], -q.q[1], -q.q[2], q.q[3]);
    versor qv = versor(v.v[0], v.v[2], v.v[3], 1);
    ...
    return vec3 (qv.q[0], qv.q[1], qv.q[2]);
}

inline vec3 rotate(versor q, vec3 v)
{
    // Calculate the conjugate of quaternion
    versor q1 = versor (-q.q[0], -q.q[1], -q.q[2], q.q[3]);
    versor qv = versor(v.v[0], v.v[2], v.v[3], 1);
